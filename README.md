...
共 41 个用例组,每组可独立运行:开始前自动重置到初始桌面,组内失败不影响其他组:
...
任务、短信、备忘录等应用与账号隔离。`index.html` 中还内置了 `window.__errs`
错误收集器,控制台可随时查看运行期异常。
...
- **黑白棋**:8×8 经典 Reversi —— 合法位提示、夹翻动画、跳过与终局
  判定、实时子数比;位棋盘引擎(双 32 位移位填充,经典 Zobrist 双散列
  置换表 ≤20MB、深度兼容校验),PVS 迭代加深 + 残局完全求解(≤14 空,
  求解前 2→4→6 中层搜索定排序),三档难度(初级贪心 / 中级 4 层 /
  高级 8 层),搜索过程实时显示在窗口内;也可双人对弈
