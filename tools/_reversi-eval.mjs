...
/** 【实验】潜在行动力:与 X 方棋子相邻的空位数。
 *  只需 8 次「走一格」而不必做完整的合法着生成(8 次 6 层填充),
 *  实测比 genMoves 便宜约 5 倍。经典黑白棋引擎常用它作为行动力的廉价替代。 */
...
function evaluate() {
  ...
  // 【实验】两次 genMoves(≈260ns)换成两次 potMob(≈50ns)
  const mobP = potMob(PLO, PHI, OLO, OHI);
  const mobO = potMob(OLO, OHI, PLO, PHI);
