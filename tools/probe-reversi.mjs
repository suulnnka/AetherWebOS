 ...
 * UI 行)钉住。开局名按名字单一化策略**可选**(2026-09-21 需求):有名局面显示
 * 「开局库 · 名字 · 估值」;无名/名字并列的局面回落「开局库 · 估值」——
 * 旧断言「首手后必带名」没跟上这份设计(失效多时)。 */
check('开局库行格式(名字可选:有名「名字 · 估值」/ 无名「估值」,均无节点段)',
  /^开局库 · [黑白]方 [+-]/.test(ai.info) || /^开局库 · [^·]+ · [黑白]方 [+-]/.test(ai.info), ai.info);
check('开局库名字若出现则是单名(名字池单一化,无「/」并列串)',
  !/^开局库 · [^·]+ · [黑白]方 [+-]/.test(ai.info) || !ai.info.includes('/'), ai.info);
