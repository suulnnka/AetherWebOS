 ...
 *
 *  【优化版】把每方向的 fill_d(落子点, O)(固定 6 层展开)换成「沿该方向走格,
 *  走到不再是 O 就停」的提前收尾循环 —— 数学等价(取 run ∩ 另一侧填充),
 *  但绝大多数方向第一步就落空(邻格非 O),总迭代次数远小于 6。 */
function moveFlips(mlo, mhi, olo, ohi) {
  let fl = 0, fh = 0, t, th, x, xh, l, h;

  // 东(+1) → ∩ fillW(P)
  t = 0; th = 0; x = ((mlo & NH) << 1) & olo; xh = ((mhi & NH) << 1) & ohi;
  while (x | xh) { t |= x; th |= xh; x = ((x & NH) << 1) & olo; xh = ((xh & NH) << 1) & ohi; }
  fl |= t & fwLo; fh |= th & fwHi;

  // 西(-1) → ∩ fillE(P)
  t = 0; th = 0; x = ((mlo & NA) >>> 1) & olo; xh = ((mhi & NA) >>> 1) & ohi;
  while (x | xh) { t |= x; th |= xh; x = ((x & NA) >>> 1) & olo; xh = ((xh & NA) >>> 1) & ohi; }
  fl |= t & feLo; fh |= th & feHi;

  // 南(+8) → ∩ fillN(P)
  t = 0; th = 0; x = (mlo << 8) & olo; xh = ((mhi << 8) | (mlo >>> 24)) & ohi;
  while (x | xh) {
    t |= x; th |= xh;
    const nx = (x << 8) & olo, nxh = ((xh << 8) | (x >>> 24)) & ohi; x = nx; xh = nxh;
  }
  fl |= t & fnLo; fh |= th & fnHi;

  // 北(-8) → ∩ fillS(P)
  t = 0; th = 0; x = ((mlo >>> 8) | (mhi << 24)) & olo; xh = (mhi >>> 8) & ohi;
  while (x | xh) {
    t |= x; th |= xh;
    const nx = ((x >>> 8) | (xh << 24)) & olo, nxh = (xh >>> 8) & ohi; x = nx; xh = nxh;
  }
  fl |= t & fsLo; fh |= th & fsHi;

  // 西南(+7) → ∩ fillNE(P)
  t = 0; th = 0; l = mlo & NA; h = mhi & NA;
  x = (l << 7) & olo; xh = ((h << 7) | (l >>> 25)) & ohi;
  while (x | xh) {
    t |= x; th |= xh; l = x & NA; h = xh & NA;
    const nx = (l << 7) & olo, nxh = ((h << 7) | (l >>> 25)) & ohi; x = nx; xh = nxh;
  }
  fl |= t & fneLo; fh |= th & fneHi;

  // 东北(-7) → ∩ fillSW(P)
  t = 0; th = 0; l = mlo & NH; h = mhi & NH;
  x = ((l >>> 7) | (h << 25)) & olo; xh = (h >>> 7) & ohi;
  while (x | xh) {
    t |= x; th |= xh; l = x & NH; h = xh & NH;
    const nx = ((l >>> 7) | (h << 25)) & olo, nxh = (h >>> 7) & ohi; x = nx; xh = nxh;
  }
  fl |= t & fswLo; fh |= th & fswHi;

  // 东南(+9) → ∩ fillNW(P)
  t = 0; th = 0; l = mlo & NH; h = mhi & NH;
  x = (l << 9) & olo; xh = ((h << 9) | (l >>> 23)) & ohi;
  while (x | xh) {
    t |= x; th |= xh; l = x & NH; h = xh & NH;
    const nx = (l << 9) & olo, nxh = ((h << 9) | (l >>> 23)) & ohi; x = nx; xh = nxh;
  }
  fl |= t & fnwLo; fh |= th & fnwHi;

  // 西北(-9) → ∩ fillSE(P)
  t = 0; th = 0; l = mlo & NA; h = mhi & NA;
  x = ((l >>> 9) | (h << 23)) & olo; xh = (h >>> 9) & ohi;
  while (x | xh) {
    t |= x; th |= xh; l = x & NA; h = xh & NA;
    const nx = ((l >>> 9) | (h << 23)) & olo, nxh = (h >>> 9) & ohi; x = nx; xh = nxh;
  }
  fl |= t & fseLo; fh |= th & fseHi;

  _lo = fl; _hi = fh;
}

/** 【惰性版】就地走格计算落点 sq 的翻子掩码 → _lo/_hi。
 *  与 moveFlips 的唯一区别:自己判断「走完 O 之后紧跟的是不是 P」,
 *  因此不依赖 genMoves 缓存的 8 个填充 —— 可以在任意时刻、只为真正要搜的
 *  着法调用(而不是开局先把全部合法着的翻子都算一遍)。 */
function moveFlipsLazy(sq) {
  ...
  t = 0; th = 0; x = ((mlo & NH) << 1) & olo; xh = ((mhi & NH) << 1) & ohi;
  while (x | xh) { t |= x; th |= xh; x = ((x & NH) << 1) & olo; xh = ((xh & NH) << 1) & ohi; }
  if ((((t & NH) << 1) & plo) | (((th & NH) << 1) & phi)) { fl |= t; fh |= th; }

  // 西(-1)
  t = 0; th = 0; x = ((mlo & NA) >>> 1) & olo; xh = ((mhi & NA) >>> 1) & ohi;
  while (x | xh) { t |= x; th |= xh; x = ((x & NA) >>> 1) & olo; xh = ((xh & NA) >>> 1) & ohi; }
  if ((((t & NA) >>> 1) & plo) | (((th & NA) >>> 1) & phi)) { fl |= t; fh |= th; }
  ...
  if ((((t << 8) & plo) | (((th << 8) | (t >>> 24)) & phi))) { fl |= t; fh |= th; }
  ...
  if (((((t >>> 8) | (th << 24)) & plo) | ((th >>> 8) & phi))) { fl |= t; fh |= th; }
  ...
  l = t & NA; h = th & NA;
  if ((((l << 7) & plo) | (((h << 7) | (l >>> 25)) & phi))) { fl |= t; fh |= th; }
  ...
  l = t & NH; h = th & NH;
  if (((((l >>> 7) | (h << 25)) & plo) | ((h >>> 7) & phi))) { fl |= t; fh |= th; }
  ...
  l = t & NH; h = th & NH;
  if ((((l << 9) & plo) | (((h << 9) | (l >>> 23)) & phi))) { fl |= t; fh |= th; }
  ...
  l = t & NA; h = th & NA;
  if (((((l >>> 9) | (h << 23)) & plo) | ((h >>> 9) & phi))) { fl |= t; fh |= th; }
...
function search(depth, alpha, beta, player, ply, exact) {
  ...
  /* 【惰性版】只收集落点,静态排序分 = 位置权重(不再预先算全部翻子) */
  ...
  while (mlo | mhi) {
    ...
    moves[n] = sq; scores[n] = W64[sq];
    n++;
  }
  /* 插入排序(降序):只动 moves/scores 两个数组,翻子留到真正搜的时候算 */
  for (let i = 1; i < n; i++) {
  ...
  if (ttMove >= 0 && moves[0] !== ttMove) {
    for (let i = 1; i < n; i++) {
      ...
      for (let j = i; j > 0; j--) { moves[j] = moves[j - 1]; scores[j] = scores[j - 1]; }
      moves[0] = ttMove;
      break;
    }
  }
  ...
  const a0 = alpha;
  for (let i = 0; i < n; i++) {
    // 只有真正要搜的着法才计算翻子(绝大多数节点在头 1-2 个着法就截断了)
    moveFlipsLazy(moves[i]);
    fll[i] = _lo; flh[i] = _hi;
    makeMove(i, ply);
    let v;
    if (i === 0) {
...
if (MODE === 'verify') {
  let bad = 0, tested = 0;
  for (let k = 0; k < 240; k++) {
    const b = randomPosition(k);
    toBitboard(b, 'w');
    genMoves(PLO, PHI, OLO, OHI); // 先建好 P 侧方向填充(供对照版 moveFlips 使用)
    let mlo = MLO, mhi = MHI;
    while (mlo | mhi) {
      let bit, sq;
      if (mlo) { bit = mlo & -mlo; mlo ^= bit; sq = 31 - Math.clz32(bit); }
      else { bit = mhi & -mhi; mhi ^= bit; sq = 63 - Math.clz32(bit); }
      moveFlips(sq < 32 ? bit : 0, sq < 32 ? 0 : bit, OLO, OHI);
      const rlo = _lo, rhi = _hi;
      moveFlipsLazy(sq);
      tested++;
      if (rlo !== _lo || rhi !== _hi) {
        bad++;
        if (bad <= 5) console.log(`不一致 sq=${moveName(sq)} 期望 ${rlo},${rhi} 实得 ${_lo},${_hi}`);
      }
    }
  }
  console.log(`翻子一致性校验:共 ${tested} 个着法,${bad} 个不一致`);
}
